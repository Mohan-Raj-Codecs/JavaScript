...
const alive_url = "http://52.66.239.139:81/api/alive";
...
function load_dead() {
  ...
  document.getElementById("dead-gif").style.display = "inline";
  init();
}
``
...
var aliveCheck = async () => {
  try {
    var dat = await fetch(alive_url);
    return await dat.json();
  } catch (err) {
    console.log("Network Connection is Poor :(");
  }
}
