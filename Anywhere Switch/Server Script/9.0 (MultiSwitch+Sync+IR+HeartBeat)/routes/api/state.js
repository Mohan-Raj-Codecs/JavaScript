...
function is_alive(update=false){   //Is_alive returns true if the heartbeat is called within every 5 seconds otherwise false
    alive = ((Date.now()-start) > 5000)?false:true;
    if(update)
        start = Date.now();
}
